        ...
        for rule in &self.config.components {
            ...
            for component_dir in component_dirs {
                ...
                for structure_item in &rule.structure {
                    let expected_path =
                        self.resolve_component_path(&component_dir, structure_item, component_name);
                    ...
                    for forbidden in &rule.forbidden {
                            ...
                            || dev_deps.map_or(false, |d| d.contains_key(forbidden))
                        {
                            violations.push(Violation {
                                ...
                                message: format!(
                                    "Forbidden dependency '{}' found in package.json",
                                    forbidden
                                ),
                                severity: Severity::Error,
...
mod tests {
    use super::*;
    use crate::plugins::rules::config::{DirectoryRule, NamingRule, SecurityRule, SizeRule};
    use std::fs;
    ...
    fn test_naming_rule_violation() {
        ...
        assert!(violations[0]
            .message
            .contains("does not match naming pattern"));
    ...
    fn test_security_rule_violation() {
        ...
        let config = RulesConfig {
            ...
            security: vec![SecurityRule {
                pattern: "*.js".to_string(),
                forbidden_patterns: vec![r#"apiKey\s*=\s*["']"#.to_string()],
                require_https: true,
        ...
        assert!(violations
            .iter()
            .any(|v| v.message.contains("forbidden pattern")));
        assert!(violations
            .iter()
            .any(|v| v.message.contains("forbidden function")));
    }
}
