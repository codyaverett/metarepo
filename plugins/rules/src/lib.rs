...
pub fn validate_project<P: AsRef<Path>>(project_path: P, config: &RulesConfig) -> Result<Vec<engine::Violation>> {
    let engine = RuleEngine::new(config.clone());
    engine.validate(project_path)
}

pub fn fix_violations<P: AsRef<Path>>(project_path: P, violations: &[engine::Violation]) -> Result<()> {
