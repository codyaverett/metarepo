...
use crate::mcp_server::{print_vscode_config, GestaltMcpServer};
...
impl McpPlugin {
    ...
    fn show_help(&self) -> Result<()> {
        let mut app = Command::new("gest mcp")
            ...
            .subcommand(Command::new("list").about("List saved MCP server configurations"))
            .subcommand(
                Command::new("remove")
            ...
            .subcommand(
                Command::new("serve").about("Run Gestalt as an MCP server exposing CLI tools"),
            )
            .subcommand(
                Command::new("config")
    ...
    async fn handle_add(&self, matches: &ArgMatches) -> Result<()> {
        ...
        let env = matches.get_many::<String>("env").map(|vals| {
    ...
    async fn handle_call_tool(&self, matches: &ArgMatches) -> Result<()> {
        let name = matches
            .get_one::<String>("name")
            .ok_or_else(|| anyhow::anyhow!("Server name or command required"))?;
        let server_args: Vec<String> = matches
    ...
    fn handle_command(&self, matches: &ArgMatches, _config: &RuntimeConfig) -> Result<()> {
        ...
        runtime.block_on(async {
            match matches.subcommand() {
                ...
                Some(("list-resources", sub_matches)) => {
                    self.handle_list_resources(sub_matches).await
                }
