...
pub fn create_project(project_path: &str, repo_url: &str, base_path: &Path) -> Result<()> {
    ...
    if !meta_file_path.exists() {
        return Err(anyhow::anyhow!(
            "No .meta file found. Run 'gest init' first."
        ));
    }
    ...
    config
        .projects
        .insert(project_path.to_string(), repo_url.to_string());
    ...
    println!(
        "\n  {} {}",
        "✅".green(),
        format!("Successfully created '{}'", project_path)
            .bold()
            .green()
    );
    println!(
...
pub fn import_project(project_path: &str, source: Option<&str>, base_path: &Path) -> Result<()> {
    ...
    config
        .projects
        .insert(project_path.to_string(), final_repo_url.clone());
    config.save_to_file(&meta_file_path)?;
    ...
    println!(
        "\n  {} {}",
        "✅".green(),
        format!("Successfully imported '{}'", project_path)
            .bold()
            .green()
    );
...
pub fn list_projects(base_path: &Path) -> Result<()> {
    ...
    if config.projects.is_empty() {
        ...
        println!(
            "  {} {}",
            "".dimmed(),
            "Use 'gest project import' to add projects".dimmed()
        );
        println!();
        return Ok(());
    }
    ...
    if project_path.exists() && project_path.join(".git").exists() && !force {
        ...
        if has_changes {
            eprintln!(
                "\n  {} {}",
                "⚠️".yellow(),
                format!("Project '{}' has uncommitted changes!", project_name)
                    .bold()
                    .yellow()
            );
            eprintln!(
                "     {} {}",
                "│".bright_black(),
                "Use --force to remove anyway (changes will be lost)".bright_red()
            );
            eprintln!(
                "     {} {}",
                "└".bright_black(),
                "Or commit/stash your changes first".bright_white()
            );
            eprintln!();
